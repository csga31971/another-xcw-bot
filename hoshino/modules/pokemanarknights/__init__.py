...
class CardRecordDAO:
    ...
    def _create_table(self):
        with self.connect() as conn:
            ...
            conn.execute(
                "CREATE TABLE IF NOT EXISTS ark_card_record"
    ...
    def get_card_num(self, gid, uid, cid):
        with self.connect() as conn:
            r = conn.execute(
                "SELECT num FROM ark_card_record WHERE gid=? AND uid=? AND cid=?", (gid, uid, cid)
            ).fetchone()
    ...
    def add_card_num(self, gid, uid, cid, increment=1):
        ...
        with self.connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO ark_card_record (gid, uid, cid, num) VALUES (?, ?, ?, ?)",
                (gid, uid, cid, num),
            )
        return num
    ...
    def get_cards_num(self, gid, uid):
        with self.connect() as conn:
            r = conn.execute(
                "SELECT cid, num FROM ark_card_record WHERE gid=? AND uid=? AND num>0", (gid, uid)
            ).fetchall()
    ...
    def get_group_ranking(self, gid, uid):
        with self.connect() as conn:
            r = conn.execute(
                "SELECT uid FROM ark_card_record WHERE gid=? AND num>0", (gid,)
            ).fetchall()
        if not r:
            return -1

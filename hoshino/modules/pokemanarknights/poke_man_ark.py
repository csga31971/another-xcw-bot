...
from . import *
from . import _ark_data, chara
from ...util import FreqLimiter, DailyNumberLimiter
from io import BytesIO


__BASE = os.path.split(os.path.realpath(__file__))
FRAME_DIR_PATH = os.path.join(__BASE[0],'image')
DIR_PATH = os.path.join(os.path.expanduser(hoshino.config.RES_DIR), 'img', 'arknights', 'unit')
DB_PATH = os.path.expanduser("~/.hoshino/poke_man_ark.db")
...
sv = Service('poke-man-ark', bundle='ark娱乐', help_='''
...
for image in image_list:
    ...
    if chara_id == 1000 or chara_id not in _ark_data.CHARA_NAME:
        continue
    star = image[14]
...
def get_chara_name(card_id):
    ...
    if 4000 > chara_id > 3000:
        chara_id -= 1000
        rarity_desc = '【超稀有】的'
    elif 3000 > chara_id > 2000 or chara_id > 4000:
        rarity_desc = '【稀有】的'
    else:
        chara_id += 1000
        rarity_desc = '【普通】的'
...
@sv.on_fullmatch(('方舟戳戳', '戳舟舟', '戳ark', 'ark戳戳'))
async def poke_back_text(bot, ev: CQEvent):
    ...
    if not poke_get_card_cooling_time_limiter.check(user_id):
        ...
        await bot.send(ev, f'{at_user}戳方舟冷却中(剩余{left_hour}小时{left_minute}分钟{left_second}秒)')
        return
...
@sv.on_prefix(('方舟查看仓库', '查看方舟放库', '方舟仓库'))

...
KEY_CONFIG_PATH =  f'{FILE_FOLDER_PATH}key_config.json'
ai_chance = Config(CONFIG_PATH)
DEFAULT_AI_CHANCE = 2   # 默认的AI回复概率
...
with open(KEY_CONFIG_PATH, 'r', encoding='utf-8') as f:
    key_config_file = json.load(f)
    app_id = key_config_file['app_id']
    app_key = key_config_file['app_key']

def getReqSign(params: dict) -> str:
    hashed_str = ''
    for key in sorted(params):
        hashed_str += key + '=' + quote_plus(str(params[key])) + '&'

...
async def download_chara_battle_voice(bot, ev: CQEvent):
    ...
    # ids = []
    logger = sv.logger
    url = f'https://redive.estertion.win/sound/vo_ci/'
    ...
    for eid in l:
        ...
        url = f'https://redive.estertion.win/sound/vo_ci/{eid}/'
        soup = BeautifulSoup(requests.get(url).text, 'html.parser')
        for a in soup.find_all('a'):
            file_name = a['href']
            if(file_name!= "../"):
                url = f'https://redive.estertion.win/sound/vo_ci/{eid}/{file_name}'
